...
type DnfRepoBundleConfig struct {
	...
	BaseURLDebug          string                    `yaml:"base-debug-url"`
	...
	baseURLDebugTemplate  *template.Template
}
...
func (b *DnfRepoBundleConfig) getBaseURL(
	...
	var baseURLTemplate template.Template
	if debug {
		baseURLTemplate = *b.baseURLDebugTemplate
	} else {
		baseURLTemplate = *b.baseURLFormatTemplate
	}
	if err := baseURLTemplate.Execute(&urlBuf, urlData); err != nil {
...
func (b *DnfRepoBundleConfig) GetDnfRepoParams(
	...
	*DnfRepoParams, error) {
	...
	baseURLDebug, err := b.getBaseURL(repoName, arch, versionOverride, errPrefix, true)
	if err != nil {
		return nil, err
	}
...
func LoadDnfConfig(cfgPath string) (*DnfConfig, error) {
	...
	for bundleName, repoBundleConfig := range config.DnfRepoBundleConfig {
		...
		t, parseErr = template.New(templateName).Parse(repoBundleConfig.BaseURLDebug)
		if parseErr != nil {
			return nil, fmt.Errorf(
				"dnfconfig.LoadDnfConfig: Error parsing baseurl %s for dnf repo-bundle %s",
				repoBundleConfig.BaseURLFormat, bundleName)
		}
		repoBundleConfig.baseURLDebugTemplate = t
